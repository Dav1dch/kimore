...
def run(mode='train'):
    ...
    if mode == 'train':
        ...
        model.train()
        loss_stat = torch.load('./trained_param/reloss.ckpt')
        loss_module.load_state_dict(loss_stat)
        loss_module.eval()
    elif mode == 'test':
        state_dict = torch.load('./trained_param/best_performance_param copy 3.pth')
        model.load_state_dict(state_dict)
        model.eval()
    else:
    ...
    # , preds_vel
                    ...
                    losses_xyz.append(loss_module(output_pred_xyz_list[j].view(1, 12, 3), ground_truth_xyz_list[j].view(1, 12, 3), torch.ones((1,12), dtype=bool).to('cuda')))
        ...
        if mode == 'train':
            ...
            # torch.save(loss_module.state_dict(), './trained_param/reloss.pth')
